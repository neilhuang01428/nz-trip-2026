...
(function () {
  ...
  /* ── Leaflet ──
     地圖不在載入時就建。一開頁就建圖＝馬上要拉圖磚、建幾百個 pin，
     首屏會卡住；而且很多人進來只看清單。改成「真的要看地圖時才建」：
     地圖捲進畫面、切到地圖模式、或從卡片點「在地圖上」時才初始化。 */
  var map = null, markers = {}, layer = null, lastList = [];
  function ensureMap() {
    ...
    mapWrap.classList.remove('is-idle');
    syncMap(lastList);
    return map;
  }
  ...
  function focusPlace(id, fromMap) {
    clearHot();
    if (!fromMap) ensureMap();
    ensureCard(id);                      // 卡片可能還在「未渲染」的批次裡
  ...
  /* 圖片：頁面載的是 images/t/*.webp（640px），原圖只當不支援 WebP 時的後備。
     原圖平均 316 KB、縮圖平均 35 KB——這是整站最有感的一項優化。 */
  function picture(src, alt, cls) {
    var thumb = src.replace(/^images\//, 'images/t/').replace(/\.jpg$/i, '.webp');
    return '<picture' + (cls ? ' class="' + cls + '"' : '') + '>' +
      '<source type="image/webp" srcset="' + esc(thumb) + '">' +
  ...
  function cardHtml(p) {
    ...
    var h = p.img ? '<div class="pl-img">' + picture(p.img, p.zh || p.name) + '</div>' : '';
  ...
  /* ── 分批渲染 ──
     310 張卡一次塞進 DOM，手機上要卡好幾秒。先畫 BATCH 張，
     捲到底部的哨兵元素時再畫下一批；搜尋或跳轉要用到後面的卡片時，
     ensureCard() 會把該卡之前的批次補畫完。 */
  var BATCH = 24;
  var shown = [];           // 目前畫面上這一輪的完整清單
  var cursor = 0;
  var sentinel = el('div', 'grid-end');
  var moreIO = null;

  function bindCard(card) {
    card.querySelectorAll('[data-focus]').forEach(function (b) {
      b.onclick = function () { focusPlace(b.dataset.focus, false); };
    });
    card.querySelectorAll('[data-deep]').forEach(function (b) {
      b.onclick = function () {
  ...
  function renderBatch(n) {
    var end = Math.min(shown.length, cursor + (n || BATCH));
    ...
    if (cursor >= shown.length) {
      ...
      sentinel.textContent = '還有 ' + (shown.length - cursor) + ' 個，繼續往下捲…';
      if (!moreIO && 'IntersectionObserver' in window) {
        moreIO = new IntersectionObserver(function (es) {
          if (es.some(function (e) { return e.isIntersecting; })) renderBatch();
        }, { rootMargin: '600px 0px' });
      }
      if (moreIO) moreIO.observe(sentinel);
      // 螢幕很高（或這一批不夠長）時，哨兵可能一直留在畫面內——
      // IntersectionObserver 只在「進出」時通知，不會再叫第二次。
      // 所以每畫完一批就自己看一眼，還看得到就再畫一批。
      setTimeout(fillWhileVisible, 30);
    }
    hintFirst(grid);
  }

  function fillWhileVisible() {
    if (cursor >= shown.length || sentinel.hidden) return;
    var r = sentinel.getBoundingClientRect();
    var vh = window.innerHeight || document.documentElement.clientHeight;
    if (r.top < vh + 600) renderBatch();      // renderBatch 會再排下一次檢查
  }

  /* 目標卡片還沒被畫出來時，把它之前的批次一次補完 */
  function ensureCard(id) {
    if (document.getElementById('pl-' + id)) return true;
    var i = -1;
    for (var k = 0; k < shown.length; k++) if (shown[k].id === id) { i = k; break; }
    if (i < 0) return false;
    renderBatch(i - cursor + 1);
    return true;
  }
  ...
  function render() {
    ...
    renderBatch();                       // 先畫前面幾張，其餘捲到再說

    syncMap(list);
  }

  /* 地圖上的 pin。popup 的 HTML 不先組——一次組三百段字串很貴，
     Leaflet 支援傳函式進去，點到哪個 pin 才組哪一段。 */
  function syncMap(list) {
    ...
    list.forEach(function (p) {
      var m = L.marker([p.lat, p.lng], { icon: pinIcon(p), title: p.name })
        .bindPopup(function () { return popupHtml(p); },
  ...
  /* 地圖延後初始化：先讓卡片畫出來，地圖稍後再補。
     手機的清單模式地圖是隱藏的，就真的等到切過去或點「在地圖上」才建；
     桌機地圖就在畫面上，所以捲到它就建，並加一個 idle 後備，
     免得 IntersectionObserver 因為任何原因沒送事件，地圖就永遠不出現。 */
  mapWrap.classList.add('is-idle');
  ...
  if (!isMobile()) {
    ...
    idle(function () { ensureMap(); }, { timeout: 1500 });
  }
...
(function () {
  ...
  function render() {
    ...
    list.forEach(function (c) {
      ...
      var h = c.img
        ? '<div class="cul-img"><picture>' +
          '<source type="image/webp" srcset="images/t/' + esc(c.img) + '.webp">' +
          '<img src="images/' + esc(c.img) + '.jpg" alt="' + esc(c.title) +
